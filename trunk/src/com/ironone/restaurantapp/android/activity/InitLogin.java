...
import com.ironone.restaurantapp.android.activity.Login.VerifyLogin;
...
import com.ironone.restaurantapp.android.synchronization.ConnectionManager;import android.app.Activity;
 ...
 *
 * @Created on Mar 19, 2014 : 3:27:53 PM
 * @author Gayanp
 */
...
public class InitLogin extends Activity {
	...
	public void loginSuccess() {
		...
		Intent i= null;
		if(isQueue) {
			i= new Intent(InitLogin.this, MainMenu.class);
		}
		else {
			i = new Intent(InitLogin.this, SetTable.class);
		}
		startActivity(i);
	...
	public void loginFailed() {
		...
		AppLog.info("Login", "loginFailed", "waiter login",
				"login failed", "waiter id = "
						+ waiterId.getText().toString());
		bar.setVisibility(View.INVISIBLE);
	}
	...
	public void loginDB() {
		...
		if (roleData.getPassword(waiterId.getText().toString())
				.trim()
			...
			loadingMessage.setText(getString(R.string.logging_to_device_failed));
			loginFailed();
		}
	}
	...
	public class VerifyLogin extends AutomaticLogin {
		...
		protected void onPostExecute(Void result) {
			...
			if (!deviceExist) {
				...
				loadingMessage.setText(getString(R.string.device_not_found) + " : "+ StaticData.deviceid);
				loginFailed();
